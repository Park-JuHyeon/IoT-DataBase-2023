-- Active: 1677570410767@@127.0.0.1@3306@sqldb
-- 사용 데이터베이스 변경
USE homeplus;

-- SELECT ALL 이따구로 하면 큰일남!
-- 한문장 마무리에는 ;으로 마침
...
-- 2. 테이블 위치
-- 3. 조건절위치
-- WHERE 절의 서브쿼리가 컬럼이 한개이상이거나 결과가 한개이상이면
-- 조건연산자로 조회할수가 없음
SELECT name
	 , height
  FROM usertbl
...
-- 정렬 (order by) : DEFAULT 값은 ASC이기 때문에 안적어도 오름차순
-- 내림차순 => DESC
SELECT name
     , `birthYear`
     , height
